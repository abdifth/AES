...
byte[] plainTextHex =
DatatypeConverter.parseHexBinary(DatatypeConverter.printHexBinary(plainText.getBytes("US-ASCII")));
int character = 0;
...
byte[] keyHex =
DatatypeConverter.parseHexBinary(DatatypeConverter.printHexBinary(keyString.getBytes("US-ASCII")));
character = 0;
for (int column = 0; column < 4; column++){
for (int row = 0; row < 4; row++){
...
DatatypeConverter.printHexBinary(plainTextHex));
ConsoleOutput.printStateMatrix(state, "State Matrix");
System.out.println("\nKey: " + keyString);
System.out.println("\nASCII Hex Conversion: " +
DatatypeConverter.printHexBinary(keyHex));
...
ConsoleOutput.printStateMatrix(state, "State Matrix:");
System.out.println("\nASCII Hex: " +
DatatypeConverter.printHexBinary(encryptedTextHex) + "\n");
}else{
